...
<div class="container-fluid">
	...
	<form method="POST" action="{{ route('admin.blogs.update', $blog) }}" enctype="multipart/form-data" id="blogForm">
		...
		@include('webadmin.blogs._form', ['blog' => $blog, 'seoMeta' => $seoMeta])

		<div class="d-flex gap-2 mb-3">
			<button type="submit" class="btn btn-primary">Update Post</button>
			<a href="{{ route('admin.blogs.index') }}" class="btn btn-outline-secondary">Cancel</a>
			@if ($blog->is_published)
				<a href="{{ route('website.blog.show', $blog->slug) }}" class="btn btn-outline-info" target="_blank">View Post</a>
			@endif
		</div>
	</form>
</div>

@include('webadmin.blogs._seo_analyzer', ['seoMeta' => $seoMeta])
@endsection
